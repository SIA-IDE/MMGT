        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    audio_tensor_ed = torch.cat(
                        [audio_tensor_pre[:, c, :] for c in context]
                    )

                    pixel_values_full_masked = [
                        torch.cat([tensor.view(2, video_length, -1)[:, c, :] for c in context]).view(-1, tensor.shape[-1])
                        for tensor in pixel_values_full_masks
                    ]

                    pixel_values_face_masked = [
                        torch.cat([tensor.view(2, video_length, -1)[:, c, :] for c in context]).view(-1, tensor.shape[-1])
                        for tensor in pixel_values_face_masks
                    ]

                    pixel_values_lip_masked = [
                        torch.cat([tensor.view(2, video_length, -1)[:, c, :] for c in context]).view(-1, tensor.shape[-1])
                        for tensor in pixel_values_lip_masks
                    ]


                    # pixel_values_full_mask = []
                    # for tensor in pixel_values_full_masks:  # 遍历原始 tensor 列表
                    #     valid_indices = [2 * c for c in context if 0 <= 2 * c < tensor.shape[0]]
                    #     if valid_indices:  # 只有在索引有效时才进行 cat 操作
                    #         pixel_values_full_mask.append(torch.cat([tensor[idx, :] for idx in valid_indices]))

                    # pixel_values_face_mask = []
                    # for tensor in pixel_values_face_masks:
                    #     valid_indices = [2 * c for c in context if 0 <= 2 * c < tensor.shape[0]]
                    #     if valid_indices:
                    #         pixel_values_face_mask.append(torch.cat([tensor[idx, :] for idx in valid_indices]))

                    # pixel_values_lip_mask = []
                    # for tensor in pixel_values_lip_masks:
                    #     valid_indices = [2 * c for c in context if 0 <= 2 * c < tensor.shape[0]]
                    #     if valid_indices:
                    #         pixel_values_lip_mask.append(torch.cat([tensor[idx, :] for idx in valid_indices]))
                    ...
                    pred = self.denoising_unet(
                            ...
                            t,
                            encoder_hidden_states=encoder_hidden_states,
                            audio_embedding=audio_tensor_ed,
                            pose_cond_fea=latent_pose_input,
                            full_mask=pixel_values_full_masked,
                            face_mask=pixel_values_face_masked,
                            body_mask=pixel_values_lip_masked,
