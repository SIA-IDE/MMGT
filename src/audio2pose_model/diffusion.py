...
# TODO 解决首帧不匹配的问题，将输出的x.shape的第二个维度变为59，将首帧加在第一个维度上
    @torch.no_grad()
    def ddim_sample(self, shape, cond_frame, cond, last_half=None, **kwargs):
    ...
    def p_losses(self, x_start, cond_frame, cond, t):
        ...
        # x_recon = torch.cat([cond_frame.unsqueeze(1), x_recon], dim=1)
        ...
        # TODO 补充一个脸部的loss函数和手部的loss函数

        # 映射到 (batch_size, T, 402) 的脸部部分
        model_out_head = model_out[:, :, 72:276]
        target_head = target[:, :, 72:276]

        # full reconstruction loss for head
        head_loss = self.loss_fn(model_out_head, target_head, reduction="none")
        head_loss = reduce(head_loss, "b ... -> b (...)", "mean")
        head_loss = head_loss * extract(self.p2_loss_weight, t, head_loss.shape)
        head_loss = head_loss.mean()



        # velocity loss for head
        target_head_v = target_head[:, 1:] - target_head[:, :-1]
        model_out_head_v = model_out_head[:, 1:] - model_out_head[:, :-1]
        v_head_loss = self.loss_fn(model_out_head_v, target_head_v, reduction="none")
        v_head_loss = reduce(v_head_loss, "b ... -> b (...)", "mean")
        v_head_loss = v_head_loss * extract(self.p2_loss_weight, t, v_head_loss.shape)
        v_head_loss = v_head_loss.mean()

        # acceleration loss for head
        target_head_a = target_head_v[:, 1:] - target_head_v[:, :-1]
        model_out_head_a = model_out_head_v[:, 1:] - model_out_head_v[:, :-1]
        a_head_loss = self.loss_fn(model_out_head_a, target_head_a, reduction="none")
        a_head_loss = reduce(a_head_loss, "b ... -> b (...)", "mean")
        a_head_loss = a_head_loss * extract(self.p2_loss_weight, t, a_head_loss.shape)
        a_head_loss = a_head_loss.mean()
        ...
        # 总体 loss 部分
        losses = (
            1.0 * loss.mean(),         # overall full reconstruction loss
            1.0 * v_loss.mean(),       # overall velocity loss
            1.0 * a_loss.mean(),       # overall acceleration loss
            head_weight * head_loss.mean(),     # head reconstruction loss with higher weight
            head_weight * v_head_loss.mean(),   # head velocity loss with higher weight
            head_weight * a_head_loss.mean()    # head acceleration loss with higher weight
        )
    ...
    def render_sample(
        ...
        for i in range(samples.shape[0]):
            np.save(f"{render_out}/{os.path.basename(name[i])[:-4]}.npy", samples[i].numpy())
