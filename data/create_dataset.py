...
from slice import *
# from scripts.extract_dwpose_from_vid import process_single_video
...
def create_dataset(opt):
    ...
    slice_data_video(f"train/wavs", f"train/videos", f"train/videos_dwpose",stride=opt.stride, length=opt.length)
    print("Slicing test data")
    slice_data_video(f"test/wavs", f"test/videos", f"test/videos_dwpose", stride=opt.stride, length=opt.length)
...
def parse_opt():
    ...
    parser.add_argument("--length", type=float, default=2, help="checkpoint")
    parser.add_argument(
        "--keypoint_folder",
        type=str,
        default="/root/node03-nfs/aaai/hallo/train_audio2_pose/data/train/keypoints",
        help="folder containing tps features",
    )
    parser.add_argument(
        "--wav_folder",
        type=str,
        default="/root/node03-nfs/aaai/hallo/train_audio2_pose/data/train/wavs",
