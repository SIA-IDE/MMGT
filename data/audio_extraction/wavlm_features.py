...
def extract_folder(src, dest):
    ...
    extract_ = partial(extract_wo_init, model, cfg, skip_completed=True, dest_dir=dest)
...
def extract_wo_init(model, cfg, fpath, skip_completed=True, dest_dir=""):
    ...
    # wavlm_feats = F.interpolate(wavlm_feats.transpose(1, 2), size=math.ceil(wavlm_feats.shape[1] * (12/ 25)), align_corners=True,
